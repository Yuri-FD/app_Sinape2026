...
\begin{frame}{Pressupostos do Processo Gaussiano}
    ...
    \begin{columns}[T]
        ...
        \begin{itemize}
            ...
            \item Covariância depende do vetor de separação \(\mathbf{h}\) (estacionariedade) e/ou da distância \(\|\mathbf{h}\|\) (isotropia);
...
\end{frame}
    ...
    \begin{frame}{Modelo Geoestatístico}
        \begin{equation*}
        Y(\bm{s}) \sim \mathcal{GP}\left(\mathbf{X}\boldsymbol{\beta}, \; C(h; \bm{\theta}) + \tau^2 \mathbb{I}(h=0)\right)
        \end{equation*}
...
\epsilon(\cdot) &\sim GP\!\left(0,\tau^2\mathbf{I}\right).
\end{aligned}
\end{equation*}
